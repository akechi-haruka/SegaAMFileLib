    ...
    class SysfileSyncRunner {
        internal static int Run(Options opts) {
            ...
            if (!File.Exists(opts.FileName1)) {
                Program.CmdLog.LogError("Specified file not found: {f}", opts.FileName1);
                return 1;
            }

            if (!File.Exists(opts.FileName2)) {
                Program.CmdLog.LogWarning("{f} does not exist, copying {f2}", opts.FileName2, opts.FileName1);
            ...
            if (opts.Differential != null) {
                if (!File.Exists(opts.Differential)) {
                    Program.CmdLog.LogError("Specified file not found: {f}", opts.Differential);
                    return 1;
                }
            ...
            if (opts.TargetFile is 0 or 1) {
                ...
                Program.CmdLog.LogInformation("File 1 saved to: {f}", opts.FileName1);
            }

            if (opts.TargetFile is 0 or 2) {
                data2 = SysData.UpdateRecord(data2, sysfile2.Backup);
                File.WriteAllBytes(opts.FileName2, data2);
                Program.CmdLog.LogInformation("File 2 saved to: {f}", opts.FileName2);
            }
        ...
        private static unsafe bool UpdateBookkeeping(SysData sysfile1, SysData sysfile2, SysData unmodified, SyncType option) {
            if (option == SyncType.NoChange) {
                Program.CmdLog.LogInformation("Bookkeeping will not be updated");
                return true;
            } else if (option == SyncType.DifferentialToFile1 || option == SyncType.DifferentialToFile2) {
                Program.CmdLog.LogInformation("Performing differential bookkeeping sync");

                if (unmodified == null) {
                    Program.CmdLog.LogError("No differential file was provided.");
                    return false;
                }
                ...
                for (int i = 0; i < 8; i++) {
                    ...
                    Program.CmdLog.LogInformation("Chute {i} added {v}", i, c);
                }
                ...
                Program.CmdLog.LogInformation("Coin-Credit added {v}", c);
                ...
                Program.CmdLog.LogInformation("E-Money-Coin added {v}", c);
                ...
                Program.CmdLog.LogInformation("E-Money-Credit added {v}", c);
                ...
                Program.CmdLog.LogInformation("Service Credit added {v}", c);
                ...
                Program.CmdLog.LogInformation("Total Coin added {v}", c);
                ...
                Program.CmdLog.LogInformation("Total Credit added {v}", c);
            ...
            for (int i = 0; i < 8; i++) {
                ...
                Program.CmdLog.LogInformation("Chute {i} synchronized to {v}", i, c);
            }
            ...
            Program.CmdLog.LogInformation("Coin-Credit synchronized to {v}", v);
            ...
            Program.CmdLog.LogInformation("E-Money-Coin synchronized to {v}", v);
            ...
            Program.CmdLog.LogInformation("E-Money-Credit synchronized to {v}", v);
            ...
            Program.CmdLog.LogInformation("Service Credit synchronized to {v}", v);
            ...
            Program.CmdLog.LogInformation("Total Coin synchronized to {v}", v);
            ...
            Program.CmdLog.LogInformation("Total Credit synchronized to {v}", v);
        ...
        private static void UpdateCredits(SysData sysfile1, SysData sysfile2, SyncType option) {
            if (option == SyncType.NoChange) {
                Program.CmdLog.LogInformation("Credits will not be updated");
                return;
            }
            ...
            Program.CmdLog.LogInformation("Credits synchronized to {v}", v);
            Program.CmdLog.LogInformation("Remain synchronized to {v}", r);
